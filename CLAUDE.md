...
2. **與現有物件比對（案號已存在 → 就地更新，不是跳過）**：新資料的案號與地址，都要和 104woo.html
   的 BASE_ITEMS 比對。**案號已存在**時不再直接跳過，改比對拍別／開標日／開標時間／底價：
   有變動就**就地更新既有物件**（見下節「流標重新排拍」），都沒變才跳過。
...
### 流標重新排拍：同案號就地更新（2026-08-22 起）
法拍物件流標後會重新排拍（一拍→二拍→三拍→四拍），**案號不變、地址不變，只有拍別／開標日／底價會變**。
舊做法把它當成全新物件開新代號，導致**同一間房子每流標一次就換一組 tinyurl，先前發出去的連結全部失效**。
（2026-08-21 實測：該批 174 筆新物件裡有 **124 筆**其實是剛被清掉的物件重新排拍，真正全新的只有 50 筆。）

現在 `publish_new.py` 遇到同案號會**就地更新**：
* **不變**：`code`（wNNN）、`short`（tinyurl）、`full`、`id`、照片、經緯度、`school`／`shop`／`traffic`、`name`、導覽鏈。
* **更新**：`auc`／`date`／`dateISO`／`time`／`price`／`unit`／`status`／`hand`／`detail`／`feat`，並重生 `index.html`（含 `narrText` 講稿）。
* **自動刪掉會過期的衍生檔**：`voice.mp3`（講稿變了）、`card.jpg`／`hero.jpg`（底價變了）
  → **更新後務必補跑語音與 `burn_price.ps1`**，否則物件頁沒有語音、卡片還印著舊底價。
* 開標時間若爬蟲給的是 `..` 之類無法解析的雜訊，且開標日有變動 → 清空（＝時間未知，`delist.js` 保守以 17:00 起算），
  不會把上一拍的舊時間留著當成新拍別的時間。
* `--no-update` 可關掉此行為，回到舊的「同案號一律跳過」。

> 這件事跟保留期是綁在一起的：物件必須**還在** BASE_ITEMS 裡，才有得更新。
> 所以 `purge_expired.py` 的保留期同步從 7 天拉長到 30 天（見下節）。
> ⚠ 若把保留期改回 7 天又不開就地更新，會出現最糟的組合：舊物件還在（案號比對到）→ 新拍別被當重複跳過
> → 物件頁停在已過期的舊拍別 → 被 `delist.js` 隱藏 → **明明還在拍的物件從網站上消失**。

### 物件生命週期：下架與刪除
1. **下架**：開標時刻 ＋ **3 小時** 自動下架（規則唯一真理源：`104woo-assets/delist.js`）。
   開標時間為 `00:00`／空／無法解析時視為「時間未知」，保守以當日 **17:00** 起算。
2. **刪除**：下架後再滿 **30 天**（2026-08-22 由 7 天改）即刪除該物件（資料夾＋總覽 BASE_ITEMS 資料），
   並自動修補相鄰物件的導覽鏈。
   ...
   * **為什麼是 30 天**：留久一點，流標重新排拍時才比對得到案號、才能就地更新而不換短網址。
     一拍到二拍實測間隔約 21～35 天，爬蟲每 8～11 天跑一次，30 天足以涵蓋。
     過期物件本來就被 `delist.js` 藏起來不會出現在總覽，留著只佔空間、不影響版面。
