...
物件從「上架 → 開標 → 下架 → 刪除」的狀態流轉：

| 轉換 | 觸發條件 | 由誰執行 |
|------|----------|----------|
| 上架 | 批次產線生成物件頁並寫入 BASE_ITEMS | `publish_new.py` |
| 下架 | 開標時刻 ＋ 3 小時（見「下架判定」） | `delist.js`（瀏覽器端即時判定） |
| 刪除 | 下架後再滿 7 天 | `purge_expired.py`（需人為執行） |

「下架」是**顯示層的即時判定**（不改資料）；「刪除」是**實際移除資料**（資料夾＋BASE_ITEMS），
並會重建相鄰物件的導覽鏈。兩者共用同一套開標時刻演算法，保留期可用 `--days` 調整。
